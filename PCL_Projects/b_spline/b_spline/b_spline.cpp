...
#include <pcl/surface/on_nurbs/fitting_surface_tdm.h>//　NURBS 切线距离最小化
#include <pcl/surface/on_nurbs/fitting_curve_2d_asdm.h>//　平方距离最小化
...
main(int argc, char *argv[])
{
	...
	pcd_file = argv[1];//　pcd文件
	//pcd_file = "test.pcd";//　pcd文件
	file_3dm = argv[2];
	...
	// ############################################################################
	// 载入点云数据　load point cloud　可视化

	printf("  loading %s\n", pcd_file.c_str());
	pcl::PointCloud<Point>::Ptr cloud_ptr(new pcl::PointCloud<Point>);
	pcl::PCLPointCloud2 cloud2;
	pcl::on_nurbs::NurbsDataSurface data;//　表面数据

	if (pcl::io::loadPCDFile(pcd_file, cloud2) == -1)
		throw std::runtime_error("  PCD file not found.");

	fromPCLPointCloud2(cloud2, *cloud_ptr);
	PointCloud2Vector3d(cloud_ptr, data.interior);//到　data内
	pcl::visualization::PointCloudColorHandlerCustom<Point> handler(cloud_ptr, 0, 255, 0);//点云
	viewer.addPointCloud<Point>(cloud_ptr, handler, "cloud_cylinder");
	printf("  %lu points in data set\n", cloud_ptr->size());

	// ############################################################################
	// fit B-spline surface　B曲线　平面

	// 参数　parameters
	unsigned order(3);
	unsigned refinement(5);
	unsigned iterations(5);//10
	unsigned mesh_resolution(128);//256
	...
	//  fit.setQuiet (false); // enable/disable debug output　调试信息输出

	// mesh 表面网格　可视化　for visualization
...
visualizeCurve(ON_NurbsCurve &curve, ON_NurbsSurface &surface, pcl::visualization::PCLVisualizer &viewer)
{
	...
	viewer.removePointCloud("cloud_cps");//去除源点云
	viewer.addPointCloud(curve_cps, "cloud_cps");//加入更新后的点云
}
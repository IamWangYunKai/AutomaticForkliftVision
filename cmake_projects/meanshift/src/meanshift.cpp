...
bool MeanShift::setInputCloud(const pcl::PointCloud<PointXYZ>::Ptr pPntCloud)
{
...
inline bool MeanShift::execMeanShiftEachPoint(const pcl::PointXYZ &in_pnt, Point &out_pnt)
{
	// Set up KDTree
	...
	while (1)
	{
		...
		for (int i = 0, pnumber = k_indices.size(); i < pnumber; ++i)
		{
			...
			pcl::PointXYZ &nbhd_pnt = mp_pointcloud->points[index];
...
bool MeanShift::process()
{
	for (size_t i = 0; i < m_size; ++i)
	{
		const pcl::PointXYZ &pnt = mp_pointcloud->points[i];
